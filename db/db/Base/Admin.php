...
abstract class Admin implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\db\\db\\Map\\AdminTableMap';
    ...
    /**
     * Initializes internal state of db\db\Base\Admin object.
     * @see applyDefaults()
     */
     ...
     * @return $this|\db\db\Admin The current object (for fluent API support)
     */
    public function setAdminId($v)
    {
        ...
        if ($this->admin_id !== $v) {
            $this->admin_id = $v;
            $this->modifiedColumns[AdminTableMap::COL_ADMIN_ID] = true;
        }
    ...
    public function setAccessLevel($v)
    {
        ...
        if ($this->access_level !== $v) {
            $this->access_level = $v;
            $this->modifiedColumns[AdminTableMap::COL_ACCESS_LEVEL] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : AdminTableMap::translateFieldName('AccessLevel', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\db\\db\\Admin'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(AdminTableMap::COL_ADMIN_ID)) {
        ...
        if ($this->isColumnModified(AdminTableMap::COL_ACCESS_LEVEL)) {
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[3], $arr)) {
            $this->setAccessLevel($arr[$keys[3]]);
        }
    }
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\db\db\Admin The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(AdminTableMap::COL_ADMIN_ID)) {
            $criteria->add(AdminTableMap::COL_ADMIN_ID, $this->admin_id);
        }
        if ($this->isColumnModified(AdminTableMap::COL_USERNAME)) {
        ...
        if ($this->isColumnModified(AdminTableMap::COL_ACCESS_LEVEL)) {
            $criteria->add(AdminTableMap::COL_ACCESS_LEVEL, $this->access_level);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \db\db\Admin (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \db\db\Admin Clone of current object.
     * @throws PropelException
     */
