...
abstract class Topic implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\db\\db\\Map\\TopicTableMap';
    ...
    /**
     * The value for the max_participants field.
     *
     * @var        int
     */
    ...
    /**
     * Initializes internal state of db\db\Base\Topic object.
     */
    ...
    /**
     * Get the [max_participants] column value.
     *
     * @return int
     */
     ...
     * @return $this|\db\db\Topic The current object (for fluent API support)
     */
    ...
    public function setTopicName($v)
    {
        ...
        if ($this->topic_name !== $v) {
            $this->topic_name = $v;
            $this->modifiedColumns[TopicTableMap::COL_TOPIC_NAME] = true;
        }
    ...
    /**
     * Set the value of [max_participants] column.
     *
     * @param int $v new value
     * @return $this|\db\db\Topic The current object (for fluent API support)
     */
    public function setMaxParticipants($v)
    {
        ...
        if ($this->max_participants !== $v) {
            $this->max_participants = $v;
            $this->modifiedColumns[TopicTableMap::COL_MAX_PARTICIPANTS] = true;
        }
    ...
    public function setCloseDate($v)
    {
        ...
        if ($this->close_date !== null || $dt !== null) {
            if ($this->close_date === null || $dt === null || $dt->format("Y-m-d") !== $this->close_date->format("Y-m-d")) {
                ...
                $this->modifiedColumns[TopicTableMap::COL_CLOSE_DATE] = true;
            }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : TopicTableMap::translateFieldName('TopicName', TableMap::TYPE_PHPNAME, $indexType)];
            $this->topic_name = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : TopicTableMap::translateFieldName('MaxParticipants', TableMap::TYPE_PHPNAME, $indexType)];
            $this->max_participants = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : TopicTableMap::translateFieldName('CloseDate', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00') {
                $col = null;
            }
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\db\\db\\Topic'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(TopicTableMap::COL_TOPIC_NAME)) {
            $modifiedColumns[':p' . $index++]  = 'topic_name';
        }
        if ($this->isColumnModified(TopicTableMap::COL_MAX_PARTICIPANTS)) {
            $modifiedColumns[':p' . $index++]  = 'max_participants';
        }
        if ($this->isColumnModified(TopicTableMap::COL_CLOSE_DATE)) {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'max_participants':
                        $stmt->bindValue($identifier, $this->max_participants, PDO::PARAM_INT);
                        break;
                    case 'close_date':
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        $result = array(
            ...
            $keys[2] => $this->getMaxParticipants(),
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[2], $arr)) {
            $this->setMaxParticipants($arr[$keys[2]]);
        }
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\db\db\Topic The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(TopicTableMap::COL_TOPIC_NAME)) {
            $criteria->add(TopicTableMap::COL_TOPIC_NAME, $this->topic_name);
        }
        if ($this->isColumnModified(TopicTableMap::COL_MAX_PARTICIPANTS)) {
            $criteria->add(TopicTableMap::COL_MAX_PARTICIPANTS, $this->max_participants);
        }
        if ($this->isColumnModified(TopicTableMap::COL_CLOSE_DATE)) {
            $criteria->add(TopicTableMap::COL_CLOSE_DATE, $this->close_date);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \db\db\Topic (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \db\db\Topic Clone of current object.
     * @throws PropelException
     */
    ...
    public function setRegistrantEvents(Collection $registrantEvents, ConnectionInterface $con = null)
    {
        ...
        foreach ($registrantEventsToDelete as $registrantEventRemoved) {
            $registrantEventRemoved->setTopic(null);
        }
     ...
     * @return ObjectCollection|ChildRegistrantEvent[] List of ChildRegistrantEvent objects
     */
    public function getRegistrantEventsJoinCountryRelatedByCountryId(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildRegistrantEventQuery::create(null, $criteria);
        $query->joinWith('CountryRelatedByCountryId', $joinBehavior);
     ...
     * @return ObjectCollection|ChildRegistrantEvent[] List of ChildRegistrantEvent objects
     */
    public function getRegistrantEventsJoinCountryRelatedByCountryDesired(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildRegistrantEventQuery::create(null, $criteria);
        $query->joinWith('CountryRelatedByCountryDesired', $joinBehavior);
    ...
    public function setTopicCountries(Collection $topicCountries, ConnectionInterface $con = null)
    {
        ...
        foreach ($topicCountriesToDelete as $topicCountryRemoved) {
            $topicCountryRemoved->setTopic(null);
        }
     ...
     * @return ObjectCollection|ChildTopicCountry[] List of ChildTopicCountry objects
     */
    public function getTopicCountriesJoinCountry(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildTopicCountryQuery::create(null, $criteria);
